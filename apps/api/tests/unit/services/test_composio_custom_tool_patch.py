...
def _invoke(tool: MagicMock, **kwargs):
...
class TestPatchedCall:
    def test_injects_user_id_into_auth_credentials(self):
    ...
    def test_works_without_access_token_in_credentials(self):
        tool = _make_tool(auth_credentials={})
    ...
    def test_passes_proxy_as_execute_request(self):
        tool = _make_tool(auth_credentials={})
        _invoke(tool, user_id="user_42", foo="bar")

        assert tool.f.call_args.kwargs["execute_request"] is tool.client.tools.proxy

    def test_validates_request_model(self):
        tool = _make_tool(auth_credentials={})
        _invoke(tool, user_id="user_42", foo="bar")
    ...
    def test_skips_credentials_when_no_toolkit(self):
